...
export default function PainelAdmin() {
  const [abaAtiva, setAbaAtiva] = useState<'movimentacao' | 'uniforme' | 'cliente' | 'usuario' | 'alertas'>('movimentacao');
  ...
  const handleCadastrarUniforme = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      estoque_critico: Number(novoUniforme.minimo_critico) || 3,
    ...
    if (error) {
      ...
      mostrarAlerta(`Uniforme cadastrado com sucesso! Código: ${codigoFinal}`);
      setNovoUniforme({
        descricao: '',
        codigo: '',
        cliente_id: '',
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <select className="p-2 text-xs border rounded-lg bg-slate-50 font-bold text-slate-800">
              <option>🔑 Administrador (Ação Uniformes)</option>
            </select>
          </div>
        </div>

        {/* Alerta de erro do Supabase se houver */}
        {erroSupabase && (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            ...
            <p className="text-2xl font-black text-slate-800 mt-1">{produtos.length}</p>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">
                    Selecione o Uniforme / Item * ({produtos.length} no banco)
                  </label>
                  <select
                    value={movItem}
                  ...
                  >
                    ...
                    {produtos.map((p, idx) => {
                      ...
                      const nomeExibicao = p.descricao || p.nome || p.titulo || `Produto #${idx + 1}`;
                  ...
                  {produtos.length === 0 ? (
                    ...
                    produtos.map((item, idx) => (
                      <tr key={item.id || idx}>
                        ...
                        <td className="p-3 font-bold text-slate-700">{item.quantidade ?? item.qtd ?? 0} un</td>
